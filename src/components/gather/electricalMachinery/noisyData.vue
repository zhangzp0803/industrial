<template>
  <div>
    ...
    <el-upload ref="upload"
               ...
               accept="audio/*"
               class="upload-demo">
      <el-button v-if="isEdit === 'edit'" icon="el-icon-upload" class="button">噪音上传</el-button>
    </el-upload>
...
export default {
  ...
  methods: {
    ...
    ...mapActions({
      ...
      delNoisyData: 'gather/delNoisyData'
    }),
    download(row) {

    },
    del(row, rowIndex) {
      this.delNoisyData({obj: row, rowIndex: rowIndex})
    },
    getURL(row) {
      return process.env.BASE_API + '/industry/downloadnoisefile?motornoiseid=' + row.motornoiseid;
    }
  },
  computed: {
    ...mapState('gather', {
      listData(state) {
        if (!state.noisyDataList) {
          return [];
        } else {
          return state.noisyDataList;
        }
      }
    }),
    url() {
      return process.env.BASE_API + '/industry/uploadMotorNoiseData/multi'
    }
  },
  data() {
    return {
