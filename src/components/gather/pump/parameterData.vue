      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          <template v-if="scope.row.edit">
            <el-input  size="small" v-model="scope.row.pumptype"></el-input>
          </template>
          <span v-else>{{ scope.row.pumptype }}</span>
        </template>
      </el-table-column>
      <el-table-column
        prop="used"
        ...
        <template slot-scope="scope">
          <template v-if="scope.row.edit">
            <el-input  size="small" v-model="scope.row.used"></el-input>
          </template>
          <span v-else>{{ scope.row.used }}</span>
        </template>
      </el-table-column>
      <el-table-column
        prop="flow"
...
export default {
  ...
  methods: {
    ...mapActions({
      ...
      updatePumpParameterData: 'gatherPump/updatePumpParameterData',
      delPumpParameterData: 'gatherPump/delPumpParameterData'
    }),
    add() {
      // 不允许同时两条在添加
      ...
      if (isAdd < 0) {
        ...
        this.updatePumpParameterData({obj: Object.assign({}, this.empty), rowIndex: 0});
      }
    },
    update(row, rowIndex) {
      ...
      this.updatePumpParameterData({obj: newObj, rowIndex: rowIndex});
    },
    edit(row, rowIndex) {
      row.edit = true;
      row.state = 'edit'
    ...
    del(row, rowIndex) {
      this.delPumpParameterData({obj: row, rowIndex: rowIndex})
    }
  ...
  computed: {
    ...mapState('gatherPump', {
      listData(state) {
        if (!state.pumpParameterDataList) {
          return [];
        } else {
          return state.pumpParameterDataList;
        }
      }
    })
  },
