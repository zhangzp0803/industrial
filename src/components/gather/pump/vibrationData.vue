...
export default {
  ...
  methods: {
    ...mapActions({
      ...
      updatePumpShockData: 'gatherPump/updatePumpShockData',
      delPumpShockData: 'gatherPump/delPumpShockData'
    }),
    add() {
      // 不允许同时两条在添加
      ...
      if (isAdd < 0) {
        ...
        this.updatePumpShockData({obj: Object.assign({}, this.empty), rowIndex: 0});
      }
    },
    update(row, rowIndex) {
      ...
      this.updatePumpShockData({obj: newObj, rowIndex: rowIndex});
    },
    edit(row, rowIndex) {
      row.edit = true;
      row.state = 'edit'
    ...
    del(row, rowIndex) {
      this.delPumpShockData({obj: row, rowIndex: rowIndex})
    }
  ...
  computed: {
    ...mapState('gatherPump', {
      listData(state) {
        if (!state.pupmShockDataList) {
          return [];
        } else {
          return state.pupmShockDataList;
        }
      }
    })
  },
